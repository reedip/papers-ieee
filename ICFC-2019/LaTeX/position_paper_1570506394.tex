...
bdobreli@redhat.com}
\and
\IEEEauthorblockN{2\textsuperscript{nd} Reedip Banerjee}
\IEEEauthorblockA{\textit{Openstack Platform} \\
\textit{Red Hat Inc.}\\
Noida, India \\
reedip.banerjee@gmail.com}
\and
...
For example,in the graph (Fig.~\ref{fig}), the $\mathrm{infrastructure\ edge}$ can
...
each other, but can replicate to the edge aggregation layer $\mathrm{EAL1}$ either
directly or via $\mathrm{EAL5}$, $\mathrm{cloudlet2}$,
...
when we mean that the latter two (EAL4/EAL5) are potential subordinates of either
of the former two. Finally, we can say that $\mathrm{EAL4}$ controls/manages
...
influence domains and does not imply state replication as a mandatory objective.
Nor does it impose bidirectional data synchronization, but rather provides an 
opportunity of one-way or bidirectional state replications and regulates a possibility
of issuing control/management operations. State replication may be performed from
subordinates to its upper associates, like sending reports. Or vice versa,
like sending directives to subordinates. While control/management
...
the same target are in conflict if they are not related by causality.It is not
necessary that the operations are atomic.
...
\begin{itemize}
  ...
  \item \textit{one-way convergence}\cite{b2} is mandatory for RTC.
\end{itemize}
